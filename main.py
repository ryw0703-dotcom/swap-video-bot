...
video_database = []         # التخزين الرئيسي لمقاطع الفيديو (file_id)
video_signatures = set()    # بصمة المقاطع (الحجم + المدة) لمنع التكرار
admin_upload_mode = False   # وضع التعبئة للأدمن
admin_added_count = 0        # عداد المقاطع المضافة
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    video = update.message.video
    file_id = video.file_id
    # تركيبة البصمة: (المدة_حجم الملف)
    sig = f"{video.duration}_{video.file_size}"

    # منع التكرار الدقيق بناءً على (file_id) أو (المدة + الحجم)
    ...
    if video_database:
        ...
        # حفظ فيديو المستخدم في السجلات
        video_database.append(file_id)
        video_signatures.add(sig)

        # حفظ بصمة الفيديو الصادر أيضاً
